...
-- Server-side admin lists (owner-gated commands run locally against other players)
...
-- CLIENT STATE
-- ════════════════════════════════════════
local CLIENT_PREFIX_LABEL = CLIENT_PREFIX
local Connections = {}
local SavedPos    = nil
...
local function inList(list, name)
    ...
    for _, v in ipairs(list) do if tostring(v):lower() == low then return true end end
    return false
end
...
local function checkRateLimit(name)
    local now = tick()
    if not rateLimit[name] then rateLimit[name] = {count=1, time=now} return true end
    if now - rateLimit[name].time > 10 then rateLimit[name] = {count=1, time=now} return true end
    rateLimit[name].count += 1
    return rateLimit[name].count <= 8
end

-- ════════════════════════════════════════
-- TARGET RESOLVER (works for both systems)
-- ════════════════════════════════════════
local function resolvePlayers(caller, str)
    ...
    local out = {}
    if str == "me"       then return {caller}
    elseif str == "all"  then return all
    elseif str == "others" then
        for _, p in ipairs(all) do if p ~= caller then table.insert(out, p) end end
    elseif str == "random" then
        local pool = {}
        for _, p in ipairs(all) do if p ~= caller then table.insert(pool, p) end end
        if #pool > 0 then return {pool[math.random(#pool)]} end
    elseif str == "nearest" then
        local best, bd = nil, math.huge
        for _, p in ipairs(all) do
            if p ~= caller and p.Character then
    ...
    elseif str == "admins" then
        for _, p in ipairs(all) do if isAdmin(p.Name) then table.insert(out, p) end end
    elseif str == "nonadmins" then
        for _, p in ipairs(all) do if not isAdmin(p.Name) then table.insert(out, p) end end
    else
        for _, p in ipairs(all) do
...
-- ════════════════════════════════════════
-- TOAST / HINT / MESSAGE / ALERT (server-admin UIs)
-- ════════════════════════════════════════
local function createToastGui(parent, text, duration, kind)
    ...
    local colors = {info=Color3.fromRGB(26,77,153), success=Color3.fromRGB(26,153,77),
                    warning=Color3.fromRGB(178,127,0), error=Color3.fromRGB(153,26,26)}
    local col = colors[kind] or colors.info
    local gui = Instance.new("ScreenGui")
    gui.Name="IronToast" gui.ResetOnSpawn=false gui.IgnoreGuiInset=true gui.Parent=parent
    local f = Instance.new("Frame")
    f.Size=UDim2.new(0,340,0,64) f.Position=UDim2.new(0.5,-170,1,0)
    f.BackgroundColor3=Color3.fromRGB(14,14,22) f.BorderSizePixel=0 f.Parent=gui
    Instance.new("UICorner",f).CornerRadius=UDim.new(0,8)
    local bar=Instance.new("Frame") bar.Size=UDim2.new(0,4,1,0) bar.BackgroundColor3=col bar.BorderSizePixel=0 bar.Parent=f
    Instance.new("UICorner",bar).CornerRadius=UDim.new(0,4)
    local lbl=Instance.new("TextLabel")
    lbl.Text=text lbl.Font=Enum.Font.Gotham lbl.TextSize=13
    lbl.TextColor3=Color3.fromRGB(220,220,230) lbl.BackgroundTransparency=1
    ...
    local prog=Instance.new("Frame")
    prog.Size=UDim2.new(1,0,0,3) prog.Position=UDim2.new(0,0,1,-3)
    prog.BackgroundColor3=col prog.BorderSizePixel=0 prog.Parent=f
    TweenService:Create(f,TweenInfo.new(0.2,Enum.EasingStyle.Quart),{Position=UDim2.new(0.5,-170,0.88,0)}):Play()
    task.spawn(function()
        local s=tick()
        while tick()-s<duration and f.Parent do prog.Size=UDim2.new(1-((tick()-s)/duration),0,0,3) task.wait(0.05) end
...
local function createHintGui(parent, text, duration)
    ...
    local gui=Instance.new("ScreenGui")
    gui.Name="IronHint" gui.ResetOnSpawn=false gui.IgnoreGuiInset=true gui.Parent=parent
    local f=Instance.new("Frame")
    f.Size=UDim2.new(0.6,0,0,44) f.Position=UDim2.new(0.2,0,-0.08,0)
    f.BackgroundColor3=Color3.fromRGB(12,12,20) f.BackgroundTransparency=0.1
    f.BorderSizePixel=2 f.BorderColor3=Color3.fromRGB(60,60,90) f.Parent=gui
    Instance.new("UICorner",f).CornerRadius=UDim.new(0,7)
    local lbl=Instance.new("TextLabel")
    lbl.Text=text lbl.Font=Enum.Font.GothamBold lbl.TextSize=14
    lbl.TextColor3=Color3.fromRGB(240,240,240) lbl.BackgroundTransparency=1
    ...
    pb.BackgroundColor3=Color3.fromRGB(80,120,255) pb.BorderSizePixel=0 pb.Parent=f
    TweenService:Create(f,TweenInfo.new(0.2,Enum.EasingStyle.Quart),{Position=UDim2.new(0.2,0,0.02,0)}):Play()
    task.spawn(function()
        local s=tick()
        while tick()-s<duration and f.Parent do pb.Size=UDim2.new(1-((tick()-s)/duration),0,0,3) task.wait(0.05) end
...
local function createMessageGui(parent, title, body, duration, kind)
    if not parent then return end
    duration=duration or 8 kind=kind or "info"
    for _, g in ipairs(parent:GetChildren()) do
        if g:IsA("ScreenGui") and g.Name=="IronMessage" then g:Destroy() end
    end
    local colors={info=Color3.fromRGB(26,77,153),success=Color3.fromRGB(20,120,60),
                  warning=Color3.fromRGB(160,110,0),error=Color3.fromRGB(140,30,30)}
    local col=colors[kind] or colors.info
    local gui=Instance.new("ScreenGui")
    gui.Name="IronMessage" gui.ResetOnSpawn=false gui.IgnoreGuiInset=true gui.Parent=parent
    local overlay=Instance.new("Frame")
    overlay.Size=UDim2.new(1,0,1,0) overlay.BackgroundColor3=Color3.new(0,0,0)
    overlay.BackgroundTransparency=0.55 overlay.BorderSizePixel=0 overlay.Parent=gui
    local main=Instance.new("Frame")
    main.Size=UDim2.new(0,460,0,320) main.Position=UDim2.new(0.5,-230,0.5,-160)
    main.BackgroundColor3=Color3.fromRGB(13,13,20) main.BorderSizePixel=0 main.Parent=gui
    Instance.new("UICorner",main).CornerRadius=UDim.new(0,10)
    local tb=Instance.new("Frame")
    tb.Size=UDim2.new(1,0,0,44) tb.BackgroundColor3=col tb.BorderSizePixel=0 tb.Parent=main
    Instance.new("UICorner",tb).CornerRadius=UDim.new(0,10)
    local fix=Instance.new("Frame") -- fix bottom corners of title bar
    fix.Size=UDim2.new(1,0,0,10) fix.Position=UDim2.new(0,0,1,-10)
    fix.BackgroundColor3=col fix.BorderSizePixel=0 fix.Parent=tb
    local tlbl=Instance.new("TextLabel")
    tlbl.Text=title tlbl.Font=Enum.Font.GothamBold tlbl.TextSize=15
    tlbl.TextColor3=Color3.fromRGB(255,255,255) tlbl.BackgroundTransparency=1
    tlbl.Size=UDim2.new(1,-14,1,0) tlbl.Position=UDim2.new(0,14,0,0)
    tlbl.TextXAlignment=Enum.TextXAlignment.Left tlbl.Parent=tb
    local sf=Instance.new("ScrollingFrame")
    sf.Size=UDim2.new(1,-20,1,-70) sf.Position=UDim2.new(0,10,0,54)
    sf.BackgroundTransparency=1 sf.BorderSizePixel=0
    sf.ScrollBarThickness=4 sf.ScrollBarImageColor3=Color3.fromRGB(80,80,120)
    sf.CanvasSize=UDim2.new(0,0,0,0) sf.AutomaticCanvasSize=Enum.AutomaticSize.Y sf.Parent=main
    local mlbl=Instance.new("TextLabel")
    mlbl.Text=body mlbl.Font=Enum.Font.Gotham mlbl.TextSize=13
    mlbl.TextColor3=Color3.fromRGB(210,210,225) mlbl.BackgroundTransparency=1
    mlbl.Size=UDim2.new(1,0,0,10) mlbl.AutomaticSize=Enum.AutomaticSize.Y
    mlbl.TextXAlignment=Enum.TextXAlignment.Left mlbl.TextYAlignment=Enum.TextYAlignment.Top
    mlbl.TextWrapped=true mlbl.Parent=sf
    local okBtn=Instance.new("TextButton")
    okBtn.Text="OK" okBtn.Font=Enum.Font.GothamBold okBtn.TextSize=13
    okBtn.TextColor3=Color3.fromRGB(255,255,255) okBtn.BackgroundColor3=col
    okBtn.BorderSizePixel=0 okBtn.Size=UDim2.new(0,90,0,28)
    okBtn.Position=UDim2.new(0.5,-45,1,-38) okBtn.Parent=main
...
-- Wrappers that send to PlayerGui
...
BarAccent.Size=UDim2.new(1,0,0,2) BarAccent.Position=UDim2.new(0,0,1,-2)
BarAccent.BackgroundColor3=Color3.fromRGB(100,60,255) BarAccent.BorderSizePixel=0 BarAccent.Parent=BarHolder

local PrefixLbl = Instance.new("TextLabel")
PrefixLbl.Text=CLIENT_PREFIX_LABEL PrefixLbl.Font=Enum.Font.GothamBold PrefixLbl.TextSize=16
PrefixLbl.TextColor3=Color3.fromRGB(100,60,255) PrefixLbl.BackgroundTransparency=1
PrefixLbl.Size=UDim2.new(0,24,1,0) PrefixLbl.Position=UDim2.new(0,8,0,0) PrefixLbl.Parent=BarHolder

local CmdBox = Instance.new("TextBox")
CmdBox.Text="" CmdBox.PlaceholderText="enter command  (RightShift to toggle)"
CmdBox.PlaceholderColor3=Color3.fromRGB(70,70,95) CmdBox.Font=Enum.Font.Gotham CmdBox.TextSize=14
CmdBox.TextColor3=Color3.fromRGB(230,230,240) CmdBox.BackgroundTransparency=1
CmdBox.Size=UDim2.new(1,-40,1,0) CmdBox.Position=UDim2.new(0,36,0,0)
CmdBox.TextXAlignment=Enum.TextXAlignment.Left CmdBox.ClearTextOnFocus=false CmdBox.Parent=BarHolder

-- Auto-complete dropdown
local AcDrop = Instance.new("Frame")
AcDrop.Name="AcDrop" AcDrop.Size=UDim2.new(1,0,0,0)
AcDrop.Position=UDim2.new(0,0,1,0) AcDrop.BackgroundColor3=Color3.fromRGB(14,14,24)
AcDrop.BorderSizePixel=0 AcDrop.ClipsDescendants=true AcDrop.ZIndex=11 AcDrop.Parent=BarHolder

local AcLayout=Instance.new("UIListLayout") AcLayout.Parent=AcDrop
local AcPad=Instance.new("UIPadding") AcPad.PaddingLeft=UDim.new(0,4) AcPad.PaddingRight=UDim.new(0,4) AcPad.Parent=AcDrop
...
local function setBarOpen(open)
    ...
    if open then task.delay(0.05, function() CmdBox:CaptureFocus() end)
    else CmdBox:ReleaseFocus() AcDrop.Size=UDim2.new(1,0,0,0) end
end
...
Panel.Name="CmdPanel" Panel.Size=UDim2.new(0,460,0,520)
Panel.Position=UDim2.new(0.5,-230,0.5,-260)
Panel.BackgroundColor3=Color3.fromRGB(10,10,17) Panel.BorderSizePixel=0
Panel.Visible=false Panel.ZIndex=20 Panel.Parent=GUI
Instance.new("UICorner",Panel).CornerRadius=UDim.new(0,10)

local PanelTop=Instance.new("Frame")
PanelTop.Size=UDim2.new(1,0,0,3) PanelTop.BackgroundColor3=Color3.fromRGB(100,60,255)
PanelTop.BorderSizePixel=0 PanelTop.Parent=Panel

local PTitle=Instance.new("TextLabel")
PTitle.Text="⚡  IronAdmin — All Commands"
PTitle.Font=Enum.Font.GothamBold PTitle.TextSize=15
PTitle.TextColor3=Color3.fromRGB(255,255,255) PTitle.BackgroundTransparency=1
PTitle.Size=UDim2.new(1,-50,0,42) PTitle.Position=UDim2.new(0,14,0,4)
PTitle.TextXAlignment=Enum.TextXAlignment.Left PTitle.Parent=Panel

local PClose=Instance.new("TextButton")
PClose.Text="✕" PClose.Font=Enum.Font.GothamBold PClose.TextSize=14
PClose.TextColor3=Color3.fromRGB(160,160,180) PClose.BackgroundTransparency=1
PClose.Size=UDim2.new(0,36,0,36) PClose.Position=UDim2.new(1,-40,0,6) PClose.Parent=Panel
PClose.MouseButton1Click:Connect(function() Panel.Visible=false end)

local PSrch=Instance.new("TextBox")
PSrch.PlaceholderText="🔍  search..." PSrch.PlaceholderColor3=Color3.fromRGB(75,75,100)
PSrch.Text="" PSrch.Font=Enum.Font.Gotham PSrch.TextSize=13
PSrch.TextColor3=Color3.fromRGB(210,210,230) PSrch.BackgroundColor3=Color3.fromRGB(18,18,28)
PSrch.BorderSizePixel=0 PSrch.Size=UDim2.new(1,-20,0,32) PSrch.Position=UDim2.new(0,10,0,48)
PSrch.ClearTextOnFocus=false PSrch.Parent=Panel
Instance.new("UICorner",PSrch).CornerRadius=UDim.new(0,7)
local sp2=Instance.new("UIPadding") sp2.PaddingLeft=UDim.new(0,10) sp2.Parent=PSrch

local TabRow=Instance.new("Frame")
TabRow.Size=UDim2.new(1,-20,0,28) TabRow.Position=UDim2.new(0,10,0,86)
TabRow.BackgroundColor3=Color3.fromRGB(18,18,28) TabRow.BorderSizePixel=0 TabRow.Parent=Panel
...
TabLayout.Padding=UDim.new(0,2) TabLayout.Parent=TabRow

local PList=Instance.new("ScrollingFrame")
PList.Size=UDim2.new(1,-20,1,-126) PList.Position=UDim2.new(0,10,0,120)
PList.BackgroundTransparency=1 PList.BorderSizePixel=0
PList.ScrollBarThickness=3 PList.ScrollBarImageColor3=Color3.fromRGB(100,60,255)
PList.CanvasSize=UDim2.new(0,0,0,0) PList.AutomaticCanvasSize=Enum.AutomaticSize.Y PList.Parent=Panel
...
PTitle.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 then pDrag=true pDS=i.Position pSP=Panel.Position end
end)
...
local CMDS = {
    ...
    -- SERVER COMMANDS (prefix !, owner-gated)
...
local function makePanelTab(name)
    ...
    btn.BorderSizePixel=0 btn.Size=UDim2.new(0,70,0,24) btn.AutoButtonColor=false
    btn.Parent=TabRow Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
    tabBtns[name]=btn
...
-- Set All active
TweenService:Create(tabBtns["All"],TweenInfo.new(0),{BackgroundColor3=Color3.fromRGB(100,60,255),TextColor3=Color3.fromRGB(255,255,255)}):Play()
...
function buildPanel(filter)
    ...
    for _, cmd in ipairs(CMDS) do
        local modeOk = activeTab=="All" or (activeTab=="Client" and cmd.mode=="client") or (activeTab=="Server" and cmd.mode=="server")
        if not modeOk then continue end
        local query=(cmd.name.." "..cmd.desc):lower()
        if filter and filter~="" and not query:find(filter:lower()) then continue end
        if cmd.cat~=lastCat then
            lastCat=cmd.cat
            local cl=Instance.new("TextLabel")
            cl.Text=(cmd.mode=="server" and "! " or "; ")..cmd.cat:upper()
            cl.Font=Enum.Font.GothamBold cl.TextSize=10
            cl.TextColor3=CAT_COLORS[cmd.cat] or Color3.fromRGB(150,150,150)
            cl.BackgroundTransparency=1 cl.Size=UDim2.new(1,0,0,20)
            cl.TextXAlignment=Enum.TextXAlignment.Left cl.Parent=PList
        end
        local row=Instance.new("Frame")
        row.Size=UDim2.new(1,0,0,30) row.BackgroundColor3=Color3.fromRGB(16,16,26)
        row.BorderSizePixel=0 row.Parent=PList
        Instance.new("UICorner",row).CornerRadius=UDim.new(0,6)
        local dot=Instance.new("Frame")
        dot.Size=UDim2.new(0,4,0,4) dot.Position=UDim2.new(0,7,0.5,-2)
        dot.BackgroundColor3=CAT_COLORS[cmd.cat] or Color3.fromRGB(150,150,150)
        dot.BorderSizePixel=0 dot.Parent=row Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)
        ...
        nl.BackgroundTransparency=1 nl.Size=UDim2.new(0.48,0,1,0) nl.Position=UDim2.new(0,16,0,0)
        nl.TextXAlignment=Enum.TextXAlignment.Left nl.Parent=row
        local dl=Instance.new("TextLabel")
        dl.Text=cmd.desc dl.Font=Enum.Font.Gotham dl.TextSize=11
        dl.TextColor3=Color3.fromRGB(130,130,150) dl.BackgroundTransparency=1
        dl.Size=UDim2.new(0.5,0,1,0) dl.Position=UDim2.new(0.48,0,0,0)
        dl.TextXAlignment=Enum.TextXAlignment.Left dl.Parent=row
        local cb=Instance.new("TextButton")
        cb.Text="" cb.BackgroundTransparency=1 cb.Size=UDim2.new(1,0,1,0) cb.Parent=row
        cb.MouseButton1Click:Connect(function()
            Panel.Visible=false
            if cmd.mode=="client" then
                setBarOpen(true) CmdBox.Text=cmd.name.." "
            else
                setBarOpen(true) CmdBox.Text="!"..cmd.name.." "
            end
        end)
...
local function updateAutocomplete(text)
    ...
    if text=="" then AcDrop.Size=UDim2.new(1,0,0,0) return end
    local matches,isServer={},text:sub(1,1)=="!"
    ...
    if #matches==0 then AcDrop.Size=UDim2.new(1,0,0,0) return end
    for _, cmd in ipairs(matches) do
        ...
        btn.TextXAlignment=Enum.TextXAlignment.Left btn.ZIndex=12 btn.Parent=AcDrop
        local ip=Instance.new("UIPadding") ip.PaddingLeft=UDim.new(0,8) ip.Parent=btn
        btn.MouseEnter:Connect(function() btn.BackgroundTransparency=0 btn.BackgroundColor3=Color3.fromRGB(25,25,40) end)
...
local function applyMovement()
    ...
    if h then h.WalkSpeed=State.ws h.JumpPower=State.jp h.HipHeight=State.hh end
...
local function startFly()
    ...
    bv.Name="IronFlyBV" bv.Velocity=Vector3.zero bv.MaxForce=Vector3.new(1e5,1e5,1e5) bv.Parent=hrp2
    local bg=Instance.new("BodyGyro")
    bg.Name="IronFlyBG" bg.MaxTorque=Vector3.new(1e5,1e5,1e5) bg.P=2e4 bg.Parent=hrp2
    ...
    Connections.fly=RunService.Heartbeat:Connect(function()
        if not State.fly then
            ...
            Connections.fly:Disconnect() Connections.fly=nil return
        end
...
local function stopFly() State.fly=false end
...
local function buildESP(player)
    ...
    hl.Name="IronESP_HL" hl.FillColor=Color3.fromRGB(255,60,60) hl.OutlineColor=Color3.fromRGB(255,255,255)
    hl.FillTransparency=0.55 hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop
    local bb=Instance.new("BillboardGui")
    bb.Name="IronESP_BB" bb.Size=UDim2.new(0,130,0,40) bb.StudsOffset=Vector3.new(0,3,0) bb.AlwaysOnTop=true
    local nl=Instance.new("TextLabel")
    nl.Text=player.Name.." ["..player.DisplayName.."]" nl.Font=Enum.Font.GothamBold nl.TextSize=12
    nl.TextColor3=Color3.fromRGB(255,255,255) nl.BackgroundTransparency=1
    nl.Size=UDim2.new(1,0,0.5,0) nl.TextStrokeTransparency=0.4 nl.Parent=bb
    local dl=Instance.new("TextLabel")
    dl.Font=Enum.Font.Gotham dl.TextSize=10 dl.TextColor3=Color3.fromRGB(200,200,200)
    dl.BackgroundTransparency=1 dl.Size=UDim2.new(1,0,0.5,0) dl.Position=UDim2.new(0,0,0.5,0)
    dl.TextStrokeTransparency=0.5 dl.Parent=bb
    ...
    attach() player.CharacterAdded:Connect(attach)
...
local function startFreecam()
    ...
    FreecamPart.Name="IronFreecam" FreecamPart.Anchored=true FreecamPart.CanCollide=false
    FreecamPart.Transparency=1 FreecamPart.CFrame=Cam.CFrame FreecamPart.Parent=workspace
    Cam.CameraType=Enum.CameraType.Scriptable Cam.CameraSubject=FreecamPart
    Connections.freecam=RunService.Heartbeat:Connect(function(dt)
        ...
        if dir.Magnitude>0 then
            FreecamPart.CFrame=CFrame.new(FreecamPart.Position+dir.Unit*50*dt)*(Cam.CFrame-Cam.CFrame.Position)
        end
...
local function execClient(raw)
    ...
    -- strip server prefix if accidentally sent here
    ...
    elseif cmd=="fly" then startFly() notify("Fly","Enabled — WASD + Space/Ctrl")
    elseif cmd=="unfly" then stopFly() notify("Fly","Disabled.")
    elseif cmd=="noclip" then State.noclip=true notify("Noclip","Enabled — ;clip to stop")
    elseif cmd=="clip" then State.noclip=false notify("Noclip","Disabled.")
    elseif cmd=="tppos" then
        local x,y,z=tonumber(args[1]),tonumber(args[2]),tonumber(args[3])
        if x and y and z and HRP then HRP.CFrame=CFrame.new(x,y,z) notify("TpPos",x..","..y..","..z)
        else notify("TpPos","Usage: tppos x y z") end
    ...
    elseif cmd=="god" then enableGod() notify("God","ON")
    elseif cmd=="ungod" then disableGod() notify("God","OFF")
    ...
    elseif cmd=="respawn" then LP:LoadCharacter() notify("Respawn","Respawning...")
    elseif cmd=="refresh" then
        local cf=HRP and HRP.CFrame LP:LoadCharacter()
        if cf then
            LP.CharacterAdded:Wait() task.wait(0.2)
    ...
    elseif cmd=="sit" then
        local h=LP.Character and LP.Character:FindFirstChild("Humanoid")
        if h then h.Sit=true end notify("Sit","Sitting.")
    ...
    elseif cmd=="vis" then
        ...
        notify("Vis","Visible.")
    elseif cmd=="esp" then enableESP() notify("ESP","Highlights + info ON.")
    elseif cmd=="unesp" then disableESP() notify("ESP","OFF.")
    elseif cmd=="freecam" then startFreecam() notify("Freecam","ON — WASD+Space/Ctrl")
    elseif cmd=="unfreecam" then stopFreecam() notify("Freecam","OFF.")
    ...
    elseif cmd=="unview" then
        local h=LP.Character and LP.Character:FindFirstChild("Humanoid")
        if h then Cam.CameraSubject=h end notify("Unview","Camera restored.")
    ...
    elseif cmd=="unfullbright" then
        ...
        Lighting.Ambient=Color3.fromRGB(127,127,127) Lighting.FogEnd=1e6
    ...
    elseif cmd=="antiafk" then enableAntiAfk()
    ...
    elseif cmd=="copyname" then
        local t=resolvePlayers(LP,args[1])
        if #t>0 then pcall(setclipboard,t[1].Name) notify("CopyName",t[1].Name.." copied!")
    ...
    elseif cmd=="fps" then
        ...
        local s=tick()
        for i=1,10 do RunService.Heartbeat:Wait() f=f+1 end
        notify("FPS",math.floor(f/(tick()-s)).." fps")
    elseif cmd=="pos" then
        if HRP then
    ...
    elseif cmd=="hide" then
        BarHolder.Visible=false notify("IronAdmin","Bar hidden. ;show to restore.",4)
    elseif cmd=="show" then BarHolder.Visible=true notify("IronAdmin","Bar shown.")
    elseif cmd=="prefix" then
        if args[1] and #args[1]==1 then
            CLIENT_PREFIX_LABEL=args[1] PrefixLbl.Text=args[1]
    ...
    elseif cmd=="unload" then
        ...
        task.delay(0.5,function()
            ...
            disableESP() stopFly() stopFreecam() GUI:Destroy()
        end)
    else
...
local function execServer(plr, raw)
    ...
    elseif cmd=="ban" then
        ...
        for _,t in ipairs(resolvePlayers(plr,args[1])) do
            if not isOwner(t.Name) then
                ...
                t:Kick("Banned: "..reason) sendToast("Banned "..t.Name,me,3,"success")
            end
        end
    elseif cmd=="unban" then
        ...
        for i,n in ipairs(bannedPlayers) do
            if n:lower()==low then table.remove(bannedPlayers,i)
                sendToast("Unbanned "..args[1],me,3,"success") return end
        end
    ...
    elseif cmd=="unmute" then
        ...
        for i,n in ipairs(mutedPlayers) do
            if n:lower()==low then table.remove(mutedPlayers,i)
                sendToast("Unmuted "..args[1],me,3,"success") return end
        end
    ...
    elseif cmd=="jail" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "")) do
            if t.Character and t.Character:FindFirstChild("HumanoidRootPart") then
                ...
                cage.Name="IronJail_"..t.Name cage.Size=Vector3.new(10,10,10)
                cage.Position=pos+Vector3.new(0,5,0) cage.Anchored=true
                cage.CanCollide=true cage.Transparency=0.5
                cage.BrickColor=BrickColor.new("Bright blue") cage.Parent=workspace
    ...
    elseif cmd=="demote" then
        ...
        for i,n in ipairs(admins) do
            if n:lower()==low then table.remove(admins,i)
                sendToast("Demoted "..args[1],me,3,"success") return end
        end
        sendToast("Not in admin list.",me,3,"warning")
    elseif cmd=="tempadmin" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "")) do
            if not isAdmin(t.Name) then
                table.insert(tempAdmins,t.Name)
                sendToast("TempAdmin → "..t.Name,me,3,"success")
            end
        end
    elseif cmd=="mod" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "")) do
            if not isMod(t.Name) then
                table.insert(mods,t.Name) sendToast("Made "..t.Name.." mod",me,3,"success")
            end
        end
    ...
    elseif cmd=="servertp" then
        ...
        local src=resolvePlayers(plr,args[1]) local dst=resolvePlayers(plr,args[2])[1]
        ...
        if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            for _,t in ipairs(Players:GetPlayers()) do
                if t~=plr and t.Character and t.Character:FindFirstChild("HumanoidRootPart") then
                    t.Character.HumanoidRootPart.CFrame=plr.Character.HumanoidRootPart.CFrame+Vector3.new(math.random(-3,3),0,math.random(-3,3))
                end
            end
    ...
    elseif cmd=="skysend" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "")) do
            if t.Character and t.Character:FindFirstChild("HumanoidRootPart") then
                t.Character.HumanoidRootPart.CFrame=t.Character.HumanoidRootPart.CFrame+Vector3.new(0,600,0)
    ...
    elseif cmd=="unff" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "me")) do
            if t.Character then
                for _,f in ipairs(t.Character:GetChildren()) do if f:IsA("ForceField") then f:Destroy() end end
            end
        end
        sendToast("ForceField removed.",me,3,"success")
    elseif cmd=="serverfly" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "me")) do
            local ch=t.Character local hr=ch and ch:FindFirstChild("HumanoidRootPart")
            ...
            if hr and hm and not hr:FindFirstChild("SrvFlyBV") then
                ...
                local bv=Instance.new("BodyVelocity") bv.Name="SrvFlyBV"
                bv.MaxForce=Vector3.new(1e5,1e5,1e5) bv.Velocity=Vector3.zero bv.Parent=hr
                local bg=Instance.new("BodyGyro") bg.Name="SrvFlyBG"
                bg.MaxTorque=Vector3.new(1e5,1e5,1e5) bg.P=9e4 bg.CFrame=hr.CFrame bg.Parent=hr
                task.spawn(function()
                    while bv.Parent and hm.Health>0 do
                        bg.CFrame=Cam.CFrame bv.Velocity=Cam.CFrame.LookVector*50 task.wait()
                    end
                end)
    ...
    elseif cmd=="fire" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "me")) do
            if t.Character then for _,p in ipairs(t.Character:GetChildren()) do
    ...
    elseif cmd=="unfire" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "me")) do
            if t.Character then for _,p in ipairs(t.Character:GetDescendants()) do
    ...
    elseif cmd=="explode" then
        for _,t in ipairs(resolvePlayers(plr,args[1] or "me")) do
            local hr=t.Character and t.Character:FindFirstChild("HumanoidRootPart")
            if hr then local ex=Instance.new("Explosion") ex.Position=hr.Position ex.BlastPressure=0 ex.Parent=workspace end
        end
            ...
            if t.Character then
                task.spawn(function()
                    while t and t.Character and t.Parent do
                        for _,p in ipairs(t.Character:GetChildren()) do
                            if p:IsA("BasePart") then p.BrickColor=BrickColor.new(Color3.fromHSV(tick()%5/5,1,1)) end
                        end
                        task.wait(0.1)
                    end
                end)
    ...
    elseif cmd=="nogravity" then workspace.Gravity=0 sendToast("Gravity OFF.",me,3,"success")
    elseif cmd=="normalgravity" then workspace.Gravity=196.2 sendToast("Gravity reset.",me,3,"success")
    elseif cmd=="fog" then
        local d=tonumber(args[1])
        if d then Lighting.FogEnd=d sendToast("Fog → "..d,me,3,"success")
        else sendToast("Invalid.",me,3,"error") end
    elseif cmd=="nofog" then Lighting.FogEnd=1e6 sendToast("Fog removed.",me,3,"success")
    ...
    elseif cmd=="weather" then
        ...
        if w then Lighting.FogEnd=w.fe Lighting.Brightness=w.br sendToast("Weather: "..(args[1] or ""):upper(),me,3,"success")
        else sendToast("Types: clear rain storm snow fog",me,3,"error") end
    elseif cmd=="clear" then
        local n=0
        for _,o in ipairs(workspace:GetChildren()) do
            if not o:IsA("Terrain") and not o:IsA("Camera") and o~=LP.Character then o:Destroy() n=n+1 end
        end
    ...
    elseif cmd=="music" then
        ...
        if args[1]:lower()=="stop" then
            ...
            snd:Play() sendToast("Playing "..id,me,3,"success")
        end
    ...
    elseif cmd=="admins" then
        sendMessage("Admins","Owners: "..table.concat(owners,", ").."\nAdmins: "..table.concat(admins,", ")..
    ...
    elseif cmd=="party" then
        for i=1,50 do
            ...
            p.BrickColor=BrickColor.Random() p.Anchored=false p.Parent=workspace
            Debris:AddItem(p,30)
        end
...
-- ════════════════════════════════════════
-- CHAT HOOK — routes to correct executor
-- ════════════════════════════════════════
LP.Chatted:Connect(function(msg)
    ...
    elseif msg:sub(1,#CLIENT_PREFIX_LABEL)==CLIENT_PREFIX_LABEL then
        -- client command via chat (fallback)
...
CmdBox.FocusLost:Connect(function(enter)
    ...
    CmdBox.Text="" setBarOpen(false) AcDrop.Size=UDim2.new(1,0,0,0)
    if txt=="" then return end
    -- route by leading character
