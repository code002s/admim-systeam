...
-- ════════════════════════════════════════
-- GET LOCAL PLAYER (FIXED FOR SCRIPT/EXECUTOR)
-- ════════════════════════════════════════
local LP = Players.LocalPlayer
if not LP then
    LP = Players:GetPlayers()[1]
end
...
if not HRP then HRP = Char:WaitForChild("HumanoidRootPart") end
...
local function checkRateLimit(name)
    ...
    if now - rateLimit[name].time > 10 then
        rateLimit[name] = {count = 1, time = now}
        return true
    end
    rateLimit[name].count = rateLimit[name].count + 1
...
local function createToastGui(parent, text, duration, kind)
    ...
    for _, g in ipairs(parent:GetChildren()) do
        if g:IsA("ScreenGui") and g.Name == "IronToast" then
            g:Destroy()
        end
    end
...
local function createHintGui(parent, text, duration)
    ...
    for _, g in ipairs(parent:GetChildren()) do
        if g:IsA("ScreenGui") and g.Name == "IronHint" then
            g:Destroy()
        end
    end
...
local function createMessageGui(parent, title, body, duration, kind)
    ...
    for _, g in ipairs(parent:GetChildren()) do
        if g:IsA("ScreenGui") and g.Name == "IronMessage" then
            g:Destroy()
        end
    end
...
-- Wrappers that send to PlayerGui (FIXED)
...
function buildPanel(filter)
    ...
    for _, cmd in ipairs(CMDS) do
        ...
        if not modeOk then goto continue end
        local query = (cmd.name .. " " .. cmd.desc):lower()
        if filter and filter ~= "" and not query:find(filter:lower()) then goto continue end
...
PSrch:GetPropertyChangedSignal("Text"):Connect(function()
    buildPanel(PSrch.Text)
end)
buildPanel("")
...
local function enableESP()
    ...
    Connections.esp_add = Players.PlayerAdded:Connect(function(p)
        if State.esp then
            buildESP(p)
        end
    end)
...
local function serverHop()
    ...
    task.spawn(function()
        ...
        if not ok then
            notify("ServerHop", "HTTP request failed.", 3)
            return
        end
        ...
        if #servers == 0 then
            notify("ServerHop", "No open servers found.", 3)
            return
        end
        local chosen = servers[math.random(#servers)]
        notify("ServerHop", "Joining " .. chosen:sub(1, 8) .. "...", 3)
        task.delay(1, function()
            TeleportService:TeleportToPlaceInstance(pid, chosen, LP)
        end)
    end)
end
...
local function execClient(raw)
    ...
    elseif cmd == "goto" then
        ...
        if #t > 0 and t[1].Character then
            ...
            if h2 and HRP then
                HRP.CFrame = h2.CFrame + Vector3.new(0, 4, 0)
                notify("Goto", "→ " .. t[1].Name)
            end
        else
    ...
    elseif cmd == "savepos" then
        if HRP then
            SavedPos = HRP.CFrame
            notify("SavePos", "Saved.")
        end
    elseif cmd == "loadpos" then
        if SavedPos and HRP then
            HRP.CFrame = SavedPos
            notify("LoadPos", "Restored.")
        else
            notify("LoadPos", "No saved position.")
        end
    elseif cmd == "god" then
        enableGod()
        notify("God", "ON")
    ...
    elseif cmd == "esp" then
        enableESP()
        notify("ESP", "Highlights + info ON.")
    elseif cmd == "unesp" then
        disableESP()
        notify("ESP", "OFF.")
    elseif cmd == "freecam" then
        startFreecam()
        notify("Freecam", "ON — WASD+Space/Ctrl")
    ...
    elseif cmd == "view" then
        ...
        if #t > 0 and t[1].Character then
            ...
            if h then
                Cam.CameraSubject = h
                notify("View", "Spectating " .. t[1].Name)
            end
        else
            notify("View", "Player not found.")
        end
    ...
    elseif cmd == "prefix" then
        if args[1] and #args[1] == 1 then
            ...
            notify("Prefix", "Usage: prefix <char>")
        end
...
local function execServer(plr, raw)
    ...
    if not isAdmin(plr.Name) then
        sendToast("No permission.", {plr}, 3, "error")
        return
    end
    if not checkRateLimit(plr.Name) then
        sendToast("Rate limit — slow down.", {plr}, 3, "warning")
        return
    end
    local parts = {}
    for w in raw:gmatch("%S+") do
    ...
    if cmd == "kick" then
        if #args < 1 then
            sendToast("!kick <player> [reason]", me, 3, "error")
            return
        end
        local reason = table.concat(args, " ", 2)
        if reason == "" then reason = "Kicked by " .. plr.Name end
        for _, t in ipairs(resolvePlayers(plr, args[1])) do
            if not isOwner(t.Name) then
                t:Kick(reason)
                sendToast("Kicked " .. t.Name, me, 3, "success")
            else
                sendToast("Cannot kick owner.", me, 3, "warning")
            end
        end
    elseif cmd == "ban" then
        if #args < 1 then
            sendToast("!ban <player> [reason]", me, 3, "error")
            return
        end
        local reason = table.concat(args, " ", 2)
        if reason == "" then reason = "Banned by " .. plr.Name end
        for _, t in ipairs(resolvePlayers(plr, args[1])) do
            if not isOwner(t.Name) then
    ...
    elseif cmd == "unban" then
        if #args < 1 then
            sendToast("!unban <player>", me, 3, "error")
            return
        end
        ...
        for i, n in ipairs(bannedPlayers) do
            if n:lower() == low then
                ...
                sendToast("Unbanned " .. args[1], me, 3, "success")
                return
            end
        end
    ...
    elseif cmd == "unmute" then
        ...
        for i, n in ipairs(mutedPlayers) do
            if n:lower() == low then
                ...
                sendToast("Unmuted " .. args[1], me, 3, "success")
                return
            end
        end
    ...
    elseif cmd == "warn" then
        if #args < 1 then
            sendToast("!warn <player> [reason]", me, 3, "error")
            return
        end
        local reason = table.concat(args, " ", 2)
        ...
        for _, t in ipairs(resolvePlayers(plr, args[1])) do
            ...
            if warnings[t.Name] >= 3 then
                t:Kick("3 warnings reached.")
                warnings[t.Name] = nil
            end
        end
    elseif cmd == "warnings" then
        local s = "Warnings:\n"
        for n, c in pairs(warnings) do
            s = s .. n .. ": " .. c .. "/3\n"
        end
    ...
    elseif cmd == "clearwarns" then
        for _, t in ipairs(resolvePlayers(plr, args[1] or "")) do
            warnings[t.Name] = nil
            sendToast("Cleared warns for " .. t.Name, me, 3, "success")
        end
    ...
    elseif cmd == "demote" then
        ...
        for i, n in ipairs(admins) do
            if n:lower() == low then
                ...
                sendToast("Demoted " .. args[1], me, 3, "success")
                return
            end
        end
    ...
    elseif cmd == "mod" then
        for _, t in ipairs(resolvePlayers(plr, args[1] or "")) do
            if not isMod(t.Name) then
                table.insert(mods, t.Name)
                sendToast("Made " .. t.Name .. " mod", me, 3, "success")
            end
        end
    elseif cmd == "unmod" then
        ...
        for i, n in ipairs(mods) do
            if n:lower() == low then
                table.remove(mods, i)
                sendToast("Unmodded " .. args[1], me, 3, "success")
                return
            end
        end
    ...
    elseif cmd == "servertp" then
        if #args < 2 then
            sendToast("!servertp <p1> <p2>", me, 3, "error")
            return
        end
        local src = resolvePlayers(plr, args[1])
        local dst = resolvePlayers(plr, args[2])[1]
        if not dst or not dst.Character then
            sendToast("Destination not found.", me, 3, "error")
            return
        end
    ...
    elseif cmd == "ff" then
        for _, t in ipairs(resolvePlayers(plr, args[1] or "me")) do
            if t.Character then
                Instance.new("ForceField").Parent = t.Character
            end
        end
    ...
    elseif cmd == "serverunfly" then
        for _, t in ipairs(resolvePlayers(plr, args[1] or "me")) do
            ...
            if hr then
                local bv = hr:FindFirstChild("SrvFlyBV")
                if bv then bv:Destroy() end
                local bg = hr:FindFirstChild("SrvFlyBG")
        ...
        for _, t in ipairs(resolvePlayers(plr, args[1] or "me")) do
            ...
            if hr then
                for _, o in ipairs(hr:GetChildren()) do
                    if o:IsA("BodyAngularVelocity") then
                        o:Destroy()
                    end
                end
            end
    ...
    elseif cmd == "sparkle" then
        for _, t in ipairs(resolvePlayers(plr, args[1] or "me")) do
            if t.Character then
                for _, p in ipairs(t.Character:GetChildren()) do
                    if p:IsA("BasePart") then
                        Instance.new("Sparkles").Parent = p
                    end
                end
            end
        end
    ...
    elseif cmd == "color" then
        if #args < 4 then
            sendToast("!color <p> <r> <g> <b>", me, 3, "error")
            return
        end
        local r, g, b = tonumber(args[2]), tonumber(args[3]), tonumber(args[4])
        if not r then
            sendToast("Invalid RGB.", me, 3, "error")
            return
        end
    ...
    elseif cmd == "speed" then
        if #args < 2 then
            sendToast("!speed <p> <n>", me, 3, "error")
            return
        end
    ...
    elseif cmd == "message" then
        if #args < 1 then
            sendToast("!message <text>", me, 3, "error")
            return
        end
    ...
    elseif cmd == "hint" then
        if #args < 1 then
            sendToast("!hint <text>", me, 3, "error")
            return
        end
    ...
    elseif cmd == "announce" then
        if #args < 1 then
            sendToast("!announce <text>", me, 3, "error")
            return
        end
    ...
    elseif cmd == "alert" then
        if #args < 1 then
            sendToast("!alert <text>", me, 3, "error")
            return
        end
    ...
    elseif cmd == "time" then
        ...
        if h and h >= 0 and h <= 24 then
            Lighting.ClockTime = h
            sendToast("Time → " .. h .. ":00", me, 3, "success")
        else
            sendToast("0–24 only.", me, 3, "error")
        end
    elseif cmd == "gravity" then
        local g = tonumber(args[1])
        if g then
            workspace.Gravity = g
            sendToast("Gravity → " .. g, me, 3, "success")
        else
            sendToast("Invalid.", me, 3, "error")
        end
    elseif cmd == "nogravity" then
        workspace.Gravity = 0
        sendToast("Gravity OFF.", me, 3, "success")
    elseif cmd == "normalgravity" then
        workspace.Gravity = 196.2
        sendToast("Gravity reset.", me, 3, "success")
    elseif cmd == "fog" then
        local d = tonumber(args[1])
        if d then
            Lighting.FogEnd = d
            sendToast("Fog → " .. d, me, 3, "success")
        else
            sendToast("Invalid.", me, 3, "error")
        end
    elseif cmd == "nofog" then
        Lighting.FogEnd = 1e6
        sendToast("Fog removed.", me, 3, "success")
    elseif cmd == "brightness" then
        local b = tonumber(args[1])
        if b then
            Lighting.Brightness = b
            sendToast("Brightness → " .. b, me, 3, "success")
        else
            sendToast("Invalid.", me, 3, "error")
        end
    elseif cmd == "weather" then
        ...
        if w then
            Lighting.FogEnd = w.fe
            Lighting.Brightness = w.br
            sendToast("Weather: " .. (args[1] or ""):upper(), me, 3, "success")
        else
            sendToast("Types: clear rain storm snow fog", me, 3, "error")
        end
    ...
    elseif cmd == "music" then
        if not args[1] then
            sendToast("!music <id|stop>", me, 3, "error")
            return
        end
        if args[1]:lower() == "stop" then
            for _, s in ipairs(workspace:GetDescendants()) do
                if s:IsA("Sound") and s.Name == "IronMusic" then
                    s:Stop()
                    s:Destroy()
                end
            end
            ...
            if not id then
                sendToast("Invalid ID.", me, 3, "error")
                return
            end
            for _, s in ipairs(workspace:GetDescendants()) do
                if s:IsA("Sound") and s.Name == "IronMusic" then
    ...
    elseif cmd == "serverping" then
        ...
        if t then
            local ok, v = pcall(function()
                return math.floor(t:GetNetworkPing() * 1000)
            end)
    ...
    elseif cmd == "shutdown" then
        ...
        task.delay(5, function()
            for _, p in ipairs(Players:GetPlayers()) do
                p:Kick("Server shutdown by " .. plr.Name)
            end
        end)
    elseif cmd == "lock" then
        serverLocked = true
        if playerLockConn then
            playerLockConn:Disconnect()
        end
        playerLockConn = Players.PlayerAdded:Connect(function(np)
            if serverLocked then
                np:Kick("Server is locked.")
            end
        end)
...
LP.Chatted:Connect(function(msg)
    ...
    if msg:sub(1, #SERVER_PREFIX) == SERVER_PREFIX then
        if isMuted(LP.Name) then
            notify("Muted", "You are muted.")
            return
        end
